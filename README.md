...
![image](https://github.com/user-attachments/assets/a97f11e2-9e44-4321-8e44-d3f26c436a36)
 ...
 -> 'TotalCharges'컬럼을 float타입으로 변환 후 결측치를 대체해야함.

 ![image](https://github.com/user-attachments/assets/99ca02f0-13d7-4594-a01f-a95f10fdd924)

 -> TotalCharges 컬럼의 분포형태가 한쪽으로 치우쳐져있어 결측치를 중앙값으로 대체.
...
XGBoost 모델의 자체 기능 부스팅은 학습기들을 연속적으로 개선해나가는 과정에서 오차에 더 많은 가중치를 부여하여 학습을 진행하기 때문에 과적합이 발생하여 클래스 1(이탈 고객)에 대한 정확도는 낮았지만, 높은 재현율을 보여 준 것 같다.
...
### 결론:
  데이터셋이 7043개의 raw로 크기가 크지않아 딥러닝을 하기엔 적합한 데이터셋은 아니다.
  따라서 딥러닝보단 머신러닝의 성능이 좋다 예측되었고 실제로 해보니 머신러닝의 성능이 더 좋았다.
  그래도 SMOTE를 통해 Oversampling을하고 stepLR로 학습률을 조절하니 딥러닝에서도 목표했던 80%의 정확도를 이뤘다.
...
## 한 줄 회고
채연 - 쉽지 않다 .......
<br>경상 - 허허... 데이터셋을 잘 고르는것도 실력이다!
<br>미현 - 데이터 분석에 유연함이 필요하다?
<br>준   - 머신러닝에 좀 더 능숙해질 수 있는 값진 시간이었다.
